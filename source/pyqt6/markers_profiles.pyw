...
###
### old version - Konstantin Glazyrin for the needs of P02.2 beamline PETRAIII - DESY (coded under Python 2.7, PyQt version 4.10, Qt version 4.8.4)
### latest version - Konstantin Glazyrin for the needs of P02.2 beamline PETRAIII - DESY (coded under Python 3.10, PyQt6 version 6.5.3, Qt version 6.5.3)
#############
from PyQt6.QtCore import Qt, QSize, QPoint, QRect, QSettings, pyqtSignal
from PyQt6.QtGui import QPainter, QColor, QBrush, QIcon, QPixmap, QPalette, QPen, QFont, QCursor, QKeyEvent
...
RENDER_ANTIALIASING = QPainter.RenderHint.SmoothPixmapTransform
RENDER_HIGH_QUALITY = QPainter.RenderHint.Antialiasing

ANTIALIASING_HINT = RENDER_ANTIALIASING
...
version 1.0 - multiple settings file capability - first argument ot the script
PyQT6 version
...
(SETPOS, SETSIZE, SETMARSHAPE, SETMARCOLOR, SETMARMCCOLOR,
 ...
 SETGRIDUNIT, SETSCRDISTANCE, SETOBJDISTANCE) = ("pos", "size", "marker-shape", "marker-color", "move-cursor-color",
                                                 ...
                                                 "marker-move-step", "marker-move-step-small", "marker-resize-step",
...
MOVECURSORCOLORLIST = (
COLORLIGHTRED, COLORGRN, COLORBRN, COLORPRPL, COLORORG, COLORBLUE, COLORLIGHTBLUE, COLORGREEN, COLORLIGHTGREEN,
COLORRED, COLORGREY, COLORWHITE, COLORBLACK)
FRAMECOLORLIST = (
COLORGRN, COLORBRN, COLORPRPL, COLORORG, COLORBLUE, COLORLIGHTBLUE, COLORGREEN, COLORLIGHTGREEN, COLORRED, COLORGREY,
COLORWHITE, COLORBLACK)
# overlay colors
(COLOROVERLAYBKG, COLOROVERLAYBKGLINE, COLOROVERLAYLINE, COLOROVERLAYTEXT) = (
QColor(100, 100, 100, 100), QColor(100, 100, 100, 0), QColor(255, 255, 100, 255), QColor(255, 0, 0, 150))
...
FRAMESHAPELIST = (
SQRTHICK, SQRTHIN, CIRCLETHICK, CIRCLETHIN, RECTTHICK, RECTTHIN, ELLIPSETHICK, ELLIPSETHIN, BOUNDSTHICK, BOUNDSTHIN,
GRID)
...
class MarkerCanvas(QWidget):
    moveOrResize = pyqtSignal()
    moveCursorEvent = pyqtSignal()
    styleChange = pyqtSignal()
    ...
    def initVars(self):
        ...
        # antialiasing hint
        self._antialiasing = ANTIALIASING_HINT
        return
    ...
    def initSelf(self):
        ...
        pal.setColor(QPalette.ColorRole.Window, QColor(100, 100, 100, 100))
        self.setPalette(pal)
        return
    ...
    def report(self, msg):
        self.reportChange.emit("Action: %s" % msg)

    # mouse events processing
    def mousePressEvent(self, event):
        # track only left clicks
        if (event.button() == Qt.MouseButton.LeftButton):
            self._oldMousePos = event.globalPosition()
    ...
    def moveCursorGetSetPos(self, value=None):
        if (value != None):
            # direct point or tuple expected in PyQt6
            value = value
            self.cursormovepos = value
            self.moveCursorEvent.emit()
    ...
    def getSetOpacity(self, value=None):
        if (value != None):
            if isinstance(value, (list, tuple)):
                value = value[0]
            value = float(value)
            if isinstance(value, float) and value >= OPAMINIMUM and value <= OPAMAXIMUM + OPASTEP / 2:
                self.opacity = value
    ...
    # legacy converter removed; native Python lists are used in PyQt6
    def listQvarToInts(self, tlist):
        return [int(v) for v in list(tlist)]
    ...
    def cycleGrid(self, painter, i):
        if (i % 2):
            mypen = painter.pen()
            mypen.setStyle(Qt.PenStyle.DashLine)
    ...
    def drawRectBounds(self, painter, x, y, width, height, pen):
        ...
        painter.drawLine(x + int((width - boundslength) / 2), height, x + int((width + boundslength) / 2),
                         height)  # bottom
    ...
    def processMovement(self, key, modifier):
        ...
        ###
        if (not (modifier & Qt.KeyboardModifier.ControlModifier) and not (
            ...
            # setup direction
            (xdir, ydir) = (0, 0)
            if (key == Qt.Key.Key_Up or key == Qt.Key.Key_8):
                ydir = -tempstep
            elif (key == Qt.Key.Key_Down or key == Qt.Key.Key_2):
                ydir = tempstep
            elif (key == Qt.Key.Key_Left or key == Qt.Key.Key_4):
                xdir = -tempstep
            elif (key == Qt.Key.Key_Right or key == Qt.Key.Key_6):
                xdir = tempstep
            elif (key == Qt.Key.Key_End or key == Qt.Key.Key_1):
                (xdir, ydir) = (-tempstep, tempstep)
            elif (key == Qt.Key.Key_PageDown or key == Qt.Key.Key_3):
                (xdir, ydir) = (tempstep, tempstep)
            elif (key == Qt.Key.Key_Home or key == Qt.Key.Key_7):
                (xdir, ydir) = (-tempstep, -tempstep)
            elif (key == Qt.Key.Key_PageUp or key == Qt.Key.Key_9):
                (xdir, ydir) = (tempstep, -tempstep)
        ...
        ###
        if ((modifier & Qt.KeyboardModifier.ControlModifier) and not (
                modifier & Qt.KeyboardModifier.ShiftModifier) and self.bcrossmove):
    ...
    def processResize(self, key, modifier):
        ...
        if (not (modifier & Qt.KeyboardModifier.ControlModifier) and (modifier & Qt.KeyboardModifier.ShiftModifier)):
            tempstep = self.movestep
            if (modifier & Qt.KeyboardModifier.AltModifier):
    ...
    def processStyle(self, key, modifier):
        ...
        ###
        if (not (modifier & Qt.KeyboardModifier.ControlModifier) and not (
                modifier & Qt.KeyboardModifier.ShiftModifier) and not (modifier & Qt.KeyboardModifier.AltModifier)):
            ...
            if (key == Qt.Key.Key_5 or key == Qt.Key.Key_C):
        ...
        ###
        elif ((modifier & Qt.KeyboardModifier.ControlModifier) and not (
                modifier & Qt.KeyboardModifier.ShiftModifier) and not (modifier & Qt.KeyboardModifier.AltModifier)):
        ...
        ###
        elif (not (modifier & Qt.KeyboardModifier.ControlModifier) and (
                modifier & Qt.KeyboardModifier.ShiftModifier) and not (
                modifier & Qt.KeyboardModifier.AltModifier)):  # switch between colors with CTRL - requires repaint
            # switch between movable cursor colors with SHIFT TAB - requires repaint
            if (key == Qt.Key.Key_C and self.bcrossmove):
                self.setMoveCursorColor()
                brepaint = True
                brestyle = True
...
class MarkerControl(QMainWindow):
    QSETTINGS_FILE = "markers.ini"

    def __init__(self, app, ini_file=None, parent=None):
        super(MarkerControl, self).__init__(parent)

        # init settings file name
        if ini_file is not None and isinstance(ini_file, str):
            self.QSETTINGS_FILE = ini_file
    ...
    def initVariables(self, app):
        ...
        # initialize settings
        filename = os.path.join(os.path.dirname(__file__), self.QSETTINGS_FILE)
    ...
    def createMarkerTab(self, tab):
        ...
        tgrid.addWidget(self.cbvframe, 4, 1, 1, 1, Qt.AlignmentFlag.AlignLeft)
        ...
        rgrid.addWidget(self.btnmarktopsmall, 1, 2, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btnmarkleftlarge, 2, 0)
        rgrid.addWidget(self.btnmarkleftsmall, 2, 1, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btnmarkrightsmall, 2, 4, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btnmarkrightlarge, 2, 5)
        rgrid.addWidget(self.btnmarkbottomsmall, 3, 2, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btnmarkbottomlarge, 4, 2)
        self.resizeWidgetsSeq(QSize(40, 40), self.btnmarktoplarge, self.btnmarkleftlarge, self.btnmarkrightlarge,
                              self.btnmarkbottomlarge)
        ...
        for w in (self.lexmarker, self.leymarker, self.lewidthmarker, self.leheightmarker, self.lelstepmarker,
                  self.lesstepmarker):
            w.textEdited.connect(self.setInputChange)
            w.editingFinished.connect(self.updateMarker)
            w.returnPressed.connect(self.updateMarker)
        self.cbvframe.stateChanged.connect(self.setInputChange)
        self.cbvframe.stateChanged.connect(self.updateMarker)
        for w in (self.cmblstepmarker, self.cmbsstepmarker):
            w.currentIndexChanged.connect(self.setInputChange)
            w.currentIndexChanged.connect(self.updateMarker)
        ...
        self.createBtnAction(self.btnmarktoplarge, BTNSMARKERMOVE, BTNLARGESTEP, Qt.Key.Key_Up,
                             Qt.KeyboardModifier.NoModifier)
        self.createBtnAction(self.btnmarkbottomlarge, BTNSMARKERMOVE, BTNLARGESTEP, Qt.Key.Key_Down,
                             Qt.KeyboardModifier.NoModifier)
        self.createBtnAction(self.btnmarkleftlarge, BTNSMARKERMOVE, BTNLARGESTEP, Qt.Key.Key_Left,
                             Qt.KeyboardModifier.NoModifier)
        self.createBtnAction(self.btnmarkrightlarge, BTNSMARKERMOVE, BTNLARGESTEP, Qt.Key.Key_Right,
                             Qt.KeyboardModifier.NoModifier)
        # small step
        # wdgt, operation and step flag are passed for redundancy, not needed now
        self.createBtnAction(self.btnmarktopsmall, BTNSMARKERMOVE, BTNSMALLSTEP, Qt.Key.Key_Up,
                             Qt.KeyboardModifier.AltModifier)
        self.createBtnAction(self.btnmarkbottomsmall, BTNSMARKERMOVE, BTNSMALLSTEP, Qt.Key.Key_Down,
                             Qt.KeyboardModifier.AltModifier)
        self.createBtnAction(self.btnmarkleftsmall, BTNSMARKERMOVE, BTNSMALLSTEP, Qt.Key.Key_Left,
                             Qt.KeyboardModifier.AltModifier)
        self.createBtnAction(self.btnmarkrightsmall, BTNSMARKERMOVE, BTNSMALLSTEP, Qt.Key.Key_Right,
                             Qt.KeyboardModifier.AltModifier)
    ...
    def createMarkerResizeTab(self, tab):
        ...
        rgrid.addWidget(self.btnmarktopsmallresize, 1, 2, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btnmarkleftlargeresize, 2, 0)
        rgrid.addWidget(self.btnmarkleftsmallresize, 2, 1, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btnmarkrightsmallresize, 2, 4, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btnmarkrightlargeresize, 2, 5)
        rgrid.addWidget(self.btnmarkbottomsmallresize, 3, 2, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btnmarkbottomlargeresize, 4, 2)
        self.resizeWidgetsSeq(QSize(40, 40), self.btnmarktoplargeresize, self.btnmarkleftlargeresize,
                              self.btnmarkrightlargeresize, self.btnmarkbottomlargeresize)
        ...
        # assigning signals and slots in sequence
        for w in (self.lelsresizemarker, self.lessresizemarker):
            w.textEdited.connect(self.setInputChange)
            w.editingFinished.connect(self.updateMarkerResize)
            w.returnPressed.connect(self.updateMarkerResize)
        for w in (self.cmblstepmarkerresize, self.cmbsstepmarkerresize):
            w.currentIndexChanged.connect(self.setInputChange)
            w.currentIndexChanged.connect(self.updateMarkerResize)

        # assigning button actions - the idea just to forward key actions to the marker window
        # large step - in principle I pass small step flag for redundance and future possible actions, so far it is modifier which matters
        # wdgt, operation and step flag are passed for redundancy, not needed now
        # marker operates different from cursor. for general purpose we use large step instead of small
        self.createBtnAction(self.btnmarktoplargeresize, BTNSMARKERRESIZE, BTNLARGESTEP, Qt.Key.Key_Up,
                             Qt.KeyboardModifier.ShiftModifier)
        self.createBtnAction(self.btnmarkbottomlargeresize, BTNSMARKERRESIZE, BTNLARGESTEP, Qt.Key.Key_Down,
                             Qt.KeyboardModifier.ShiftModifier)
        self.createBtnAction(self.btnmarkleftlargeresize, BTNSMARKERRESIZE, BTNLARGESTEP, Qt.Key.Key_Left,
                             Qt.KeyboardModifier.ShiftModifier)
        self.createBtnAction(self.btnmarkrightlargeresize, BTNSMARKERRESIZE, BTNLARGESTEP, Qt.Key.Key_Right,
                             Qt.KeyboardModifier.ShiftModifier)
        # small step
        # wdgt, operation and step flag are passed for redundancy, not needed now
        self.createBtnAction(self.btnmarktopsmallresize, BTNSMARKERRESIZE, BTNSMALLSTEP, Qt.Key.Key_Up,
                             Qt.KeyboardModifier.AltModifier | Qt.KeyboardModifier.ShiftModifier)
        self.createBtnAction(self.btnmarkbottomsmallresize, BTNSMARKERRESIZE, BTNSMALLSTEP, Qt.Key.Key_Down,
                             Qt.KeyboardModifier.AltModifier | Qt.KeyboardModifier.ShiftModifier)
        self.createBtnAction(self.btnmarkleftsmallresize, BTNSMARKERRESIZE, BTNSMALLSTEP, Qt.Key.Key_Left,
                             Qt.KeyboardModifier.AltModifier | Qt.KeyboardModifier.ShiftModifier)
        self.createBtnAction(self.btnmarkrightsmallresize, BTNSMARKERRESIZE, BTNSMALLSTEP, Qt.Key.Key_Right,
                             Qt.KeyboardModifier.AltModifier | Qt.KeyboardModifier.ShiftModifier)
    ...
    def createCursorTab(self, tab):
        ...
        tgrid.addWidget(self.cbvcursormove, 2, 1, 1, 1, Qt.AlignmentFlag.AlignLeft)
        tgrid.addWidget(self.cbvcursor, 3, 1, 1, 1, Qt.AlignmentFlag.AlignLeft)
        ...
        rgrid.addWidget(self.btncurstopsmall, 1, 2, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btncursleftlarge, 2, 0)
        rgrid.addWidget(self.btncursleftsmall, 2, 1, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btncursrightsmall, 2, 4, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btncursrightlarge, 2, 5)
        rgrid.addWidget(self.btncursbottomsmall, 3, 2, 1, 1, Qt.AlignmentFlag.AlignCenter)
        rgrid.addWidget(self.btncursbottomlarge, 4, 2)
        self.resizeWidgetsSeq(QSize(40, 40), self.btncurstoplarge, self.btncursleftlarge, self.btncursrightlarge,
                              self.btncursbottomlarge)
        ...
        # assigning signals and slots in sequence
        for w in (self.lexcursor, self.leycursor, self.lelstepcursor, self.lesstepcursor):
            w.textEdited.connect(self.setInputChange)
            w.editingFinished.connect(self.updateCursor)
            w.returnPressed.connect(self.updateCursor)
        for w in (self.cbvcursormove, self.cbvcursor):
            w.stateChanged.connect(self.setInputChange)
            w.stateChanged.connect(self.updateCursor)
        for w in (self.cmblstepcursor, self.cmbsstepcursor):
            w.currentIndexChanged.connect(self.setInputChange)
            w.currentIndexChanged.connect(self.updateCursor)

        # assigning button actions - the idea just to forward key actions to the marker window
        # large step - in principle I pass small step flag for redundance and future possible actions, so far it is modifier which matters
        # wdgt, operation and step flag are passed for redundancy, not needed now
        # cursor operates different from marker resize and move. for general purpose we use small step instead of large
        self.createBtnAction(self.btncurstoplarge, BTNSMOVECURSOR, BTNLARGESTEP, Qt.Key.Key_Up,
                             Qt.KeyboardModifier.AltModifier | Qt.KeyboardModifier.ControlModifier)
        self.createBtnAction(self.btncursbottomlarge, BTNSMOVECURSOR, BTNLARGESTEP, Qt.Key.Key_Down,
                             Qt.KeyboardModifier.AltModifier | Qt.KeyboardModifier.ControlModifier)
        self.createBtnAction(self.btncursleftlarge, BTNSMOVECURSOR, BTNLARGESTEP, Qt.Key.Key_Left,
                             Qt.KeyboardModifier.AltModifier | Qt.KeyboardModifier.ControlModifier)
        self.createBtnAction(self.btncursrightlarge, BTNSMOVECURSOR, BTNLARGESTEP, Qt.Key.Key_Right,
                             Qt.KeyboardModifier.AltModifier | Qt.KeyboardModifier.ControlModifier)
        # small step
        # wdgt, operation and step flag are passed for redundancy, not needed now
        self.createBtnAction(self.btncurstopsmall, BTNSMOVECURSOR, BTNSMALLSTEP, Qt.Key.Key_Up,
                             Qt.KeyboardModifier.ControlModifier)
        self.createBtnAction(self.btncursbottomsmall, BTNSMOVECURSOR, BTNSMALLSTEP, Qt.Key.Key_Down,
                             Qt.KeyboardModifier.ControlModifier)
        self.createBtnAction(self.btncursleftsmall, BTNSMOVECURSOR, BTNSMALLSTEP, Qt.Key.Key_Left,
                             Qt.KeyboardModifier.ControlModifier)
        self.createBtnAction(self.btncursrightsmall, BTNSMOVECURSOR, BTNSMALLSTEP, Qt.Key.Key_Right,
                             Qt.KeyboardModifier.ControlModifier)
    ...
    def createGridTab(self, tab):
        ...
        self.sbgridunit.valueChanged.connect(self.setInputChange)
        self.sbgridunit.valueChanged.connect(self.updateGrid)
    ...
    def createCalibrationTab(self, tab):
        ...
        self.setWidgetsDisable(True, self.lecwidth, self.lecheight, self.leccursorcursor, self.leccursorx,
                               self.leccursory)
        ...
        for w in (self.ledistscreen, self.ledistobj):
            w.textChanged.connect(self.updateCalibrationTab)
            w.returnPressed.connect(self.updateCalibrationTab)

        # assign bnt click event
        self.btncalibration.clicked.connect(self.btnStartCalibration)
    ...
    def trackBtnAction(self, wdgt, operation, stepflag, key, modifier):
        kevent = QKeyEvent(QEvent.Type.KeyPress, key, modifier)
        QApplication.instance().postEvent(self.marker, kevent)
    ...
    def createBtnAction(self, wdgt, operation, stepflag, key, modifier):
        ...
        wdgt.clicked.connect(func_callback)
    ...
    def initEvents(self):
        ...
        self.marker.moveOrResize.connect(self.updateMarkerTab)
        self.marker.moveOrResize.connect(self.updateCalibrationTab)

        # movable cursor - move event - affects Cursor tab and Calibration tab
        self.marker.moveCursorEvent.connect(self.updateCursorTab)
        self.marker.moveCursorEvent.connect(self.updateCalibrationTab)

        # style change event - affects Style tab
        self.marker.styleChange.connect(self.updateStyleTab)

        # report changes
        self.marker.reportChange.connect(self.showShortMsg)
        # self.reportChange is not defined on this class
    ...
    def readSettings(self):
        ...
        try:
            ...
            self.marker.moveCursorGetSetPos(QPoint(-10, -10))

        # movable cross visibility
        value = bool(self.settings.value(SETMARMCDISPLAY, self.marker.moveCursorGetSetVisibility()))
        self.marker.moveCursorGetSetVisibility(value)
        # frame visibility
        value = bool(self.settings.value(SETDISPLAY, self.marker.getSetVisibility()))
        self.marker.getSetVisibility(value)
        # frame cursor visibility
        value = bool(self.settings.value(SETCURSORDISPLAY, self.marker.getSetCursorVisibility()))
        self.marker.getSetCursorVisibility(value)
        # marker opacity
        value = float(self.settings.value(SETOPACITY, self.marker.getSetOpacity()))
        self.marker.getSetOpacity(value)
        # marker move step
        value = int(self.settings.value(SETMOVESTEP, self.marker.getSetMoveStep()))
        self.marker.getSetMoveStep(value)
        # marker move step small
        value = int(self.settings.value(SETMOVESTEPSMALL, self.marker.getSetMoveStepSmall()))
        self.marker.getSetMoveStepSmall(value)
        # marker resize step
        value = int(self.settings.value(SETRESIZESTEP, self.marker.getSetResizeStep()))
        self.marker.getSetResizeStep(value)
        # marker resize step small
        value = int(self.settings.value(SETRESIZESTEPSMALL, self.marker.getSetResizeStepSmall()))
        self.marker.getSetResizeStepSmall(value)
        # marker move cursor move step - large
        value = int(self.settings.value(SETMARMCMOVESTEP, self.marker.moveCursorGetSetMoveStep()))
        ...
        # small grid unit
        value = int(self.settings.value(SETGRIDUNIT, self.marker.getSetGridUnit()))
        self.marker.getSetGridUnit(value)

        # calibration
        # screen distance
        value = float(self.settings.value(SETSCRDISTANCE, self.getSetScreenDistance()))
        self.getSetScreenDistance(value)
        # object  distance
        value = float(self.settings.value(SETOBJDISTANCE, self.getSetObjectDistance()))
    ...
    def updateMarkerTab(self):
        ...
        (x, y, width, height) = self.intStringSeq(self.marker.x(), self.marker.y(), self.marker.width(),
                                                  self.marker.height())
    ...
    def updateMarkerResizeTab(self):
        # update marker tab - pos, dimensions
    ...
    def updateCursorTab(self):
        # update marker tab - pos, dimensions
        (x, y, step, stepsmall) = self.intStringSeq(self.marker.moveCursorGetSetPos().x(),
                                                    self.marker.moveCursorGetSetPos().y(),
                                                    self.marker.moveCursorGetSetMoveStep(),
                                                    self.marker.moveCursorGetSetMoveStepSmall())
    ...
    def updateCalibrationTab(self):
        ...
        (width, height, cursdist, x, y) = self.floatStringSeq(
            self.massMultiply(self.objdistance / self.scrdistance, width, height, cursdist, x, y))
    ...
    def btnStartCalibration(self):
        ...
        self.overlay.calibrationCanceled.connect(self.cleanupCalibration)
        self.overlay.calibrationSuccess.connect(self.applyCalibration)
    ...
    def setCheckBoxOnBool(self, wdgt, state):
        bchecked = Qt.CheckState.Unchecked
        schecked = False
        if (state):
            bchecked = Qt.CheckState.Checked
            schecked = True

        wdgt.setChecked(schecked)
        return bchecked
    ...
    def connectSeq(self, tlist, signal, slot):
        ...
        for wdgt in temp:
            # legacy helper now simplified through direct .connect above
            pass
    ...
    def updateMarker(self):
        ...
        if (self.getInputChange()):
            ...
            # check small marker step - check if it is the same or not, update it
                ...
                v = self.cbvframe.checkState() == Qt.CheckState.Checked
    ...
    def updateCursor(self):
        ...
        if (self.getInputChange()):
            ...
            # check small marker step - check if it is the same or not, update it
                ...
                v = self.cbvcursor.checkState() == Qt.CheckState.Checked
                if (v != self.marker.getSetCursorVisibility()):
                    self.marker.getSetCursorVisibility(v)
                    self.marker.update()

                # movable cursor visibility
                v = self.cbvcursormove.checkState() == Qt.CheckState.Checked
    ...
    def trackBtnPress(self):
        k = QKeyEvent(QEvent.Type.KeyPress, Qt.Key.Key_Up, Qt.KeyboardModifier.NoModifier)
...
class MPushButton(QPushButton):
    ...
    def paintEvent(self, event):
        ...
        painter = QPainter(self)
        painter.setRenderHint(RENDER_ANTIALIASING)
...
class MOverlay(QMainWindow):
    calibrationCanceled = pyqtSignal(str)
    calibrationSuccess = pyqtSignal(float)
...
Reset calibration and clean the drawn line by pressing the right mouse button."""
    ...
    def paintEvent(self, event):
        ...
        if (len(self._message) > 0):
            ...
            rtext = painter.boundingRect(QRect(0, 0, width, height), Qt.AlignmentFlag.AlignLeft,
                                         self._message)
            rwdgt = self.rect()
            (wd, ht) = (int(rtext.width()), int(rtext.height()))

            x0, y0 = int(rwdgt.x() + (rwdgt.width() - rtext.width()) / 2), \
                      int(rwdgt.y() + (rwdgt.height() - rtext.height()) / 2)
            rtext.setRect(x0, y0, wd, ht)
            painter.drawText(rtext, Qt.AlignmentFlag.AlignLeft, self._message)
        ...
        if self._firstpos:
            if self._mode == OVERLAYCALIBRATION:
                ...
                x0, y0 = int(float(-dy) / length * endw), int(float(dx) / length * endw)
                point = QPoint(x0, y0).toPointF()

                # just to check type if necessary
                #pos1 = self._firstpos + point
                #pos2 = self._firstpos - point
                #pos3 = self._secondpos - point
                #pos4 = self._secondpos + point
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            if (self._firstpos is None):  # start of calibration
                event.accept()
                self._firstpos = event.globalPosition()
    ...
    def keyPressEvent(self, event):
        ...
        if (not self._bdraw):
            if (k == Qt.Key.Key_Escape):
                ...
                event.accept()
                self.reset()
            elif (k == Qt.Key.Key_Enter or k == Qt.Key.Key_Return):
                event.accept()
                self.prepClose()
        else:
            event.ignore()
    ...
    def mouseReleaseEvent(self, event):
        # save last point
        if (self._firstpos):
            self._secondpos = event.globalPosition()
            self._bdraw = False
        return
    ...
    def closeEvent(self, event):
        if (self._bcloseready):
            ...
            if (self._firstpos):
                ...
                self.calibrationSuccess.emit(length)
            else:
                self.calibrationCanceled.emit("Action: screen distance measurement canceled.")
        else:
            event.ignore()
        return
...
def usage():
    ...
    :return:
    """
    print(f"Usage:\npython {__file__} profile_name\n\tprofile_name - filename containing settings")


def get_settings_file():
    """
    Returns filename if given as an argument
    :return:
    """
    res = None
    if len(sys.argv) > 0:
...
if __name__ == '__main__':
    ...
    form = MarkerControl(app, ini_file=get_settings_file())
    app.exec()